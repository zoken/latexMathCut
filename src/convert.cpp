...
static bool     TranslateCommand(MathStack* mstack);	/* converts commands */
...
void Converter::ConvertString(char *string,MathStack* mstack)
/******************************************************************************
 ...
 ******************************************************************************/
{
	...
	if (PushSource(NULL, string) == 0) {
        ...
        printf("\nafter push source, before stillsource. string = %s\n",string);
		while (StillSource())
			Convert(mstack);
		PopSource();
        printf("\npop source ----------------- .\n");
...
void Converter::ConvertAllttString(char *s,MathStack* mstack)
/******************************************************************************
     purpose : converts string in TeX-format to Rtf-format
	...
	if(PushSource(NULL, s)==0) {
		...
		while (StillSource()) {
			...
			switch (cThis) {
				...
				case '}':			
					ret = RecursionLevel - mstack->PopBrace();
					fprintRTF("}");
					break;
...
void Converter::Convert(MathStack* mstack)
/****************************************************************************
...
bool TranslateCommand(MathStack* mstack)
/****************************************************************************
