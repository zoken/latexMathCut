...
CmdBibitem(int code)
{
	...
	if (label) {
		fprintRTF("[");
		converter.ConvertString(label,&mstack);
		fprintRTF("]");
		free(label);
	}
	...
	converter.ConvertString(s,&mstack);
...
/* {\field{\*\fldinst{\lang1024 REF section31 \\* MERGEFORMAT }}{\fldrslt{?}}} */
		case LABEL_CITE:
			...
			if (option) {
				fprintRTF(", ");
				converter.ConvertString(option,&mstack);
			}
			fprintRTF("]");
			free(str);
			break;
...
******************************************************************************/
{
	...
	if (code == LABEL_HTMLADDNORMALREF) {
		...
		text = getBraceParam();
		ref = getBraceParam();
		converter.ConvertString(text,&mstack);
		free(text);
		free(ref);
	}
}
